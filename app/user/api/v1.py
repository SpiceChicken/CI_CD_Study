...
# 상위 디렉토리(app)의 모듈들을 상대 경로로 임포트합니다.
from app.user import schemas, crud, models
...
def login_for_access_token_form(
    ...
    crud.create_token(db, token=refresh_token, token_type='refresh', user_id=user.id, expires_delta=timedelta(days=REFRESH_TOKEN_EXPIRE_DAYS))
...
def login_for_access_token(user_login: schemas.UserLogin, db: Session = Depends(get_db)):
    ...
    access_token = security.create_access_token(data={"sub": user.email})
    refresh_token = security.create_refresh_token(data={"sub": user.email})
    crud.create_token(db, token=refresh_token, token_type='refresh', user_id=user.id, expires_delta=REFRESH_TOKEN_EXPIRE_DAYS)
...
def refresh_access_token(token_request: schemas.RefreshTokenRequest, db: Session = Depends(get_db)):
    """
    ...
    db_token = crud.get_token_by_value(db, token=token_request.refresh_token, token_type='refresh')
    ...
    user = crud.get_user_by_id(db, user_id=db_token.user_id)
    ...
    crud.create_token(db, token=new_refresh_token_value, token_type='refresh', user_id=user.id, expires_delta=refresh_token_expires)
...
@router.post("/password-reset-request/", response_model=schemas.Message)
def request_password_reset(request: schemas.PasswordResetRequest, db: Session = Depends(get_db)):
    ...
    if user and user.is_active:
        ...
        crud.create_token(db, token=reset_token_value, token_type='reset', user_id=user.id, expires_delta=reset_token_expires)
...
def reset_password(reset_data: schemas.PasswordReset, db: Session = Depends(get_db)):
    ...
    db_token = crud.get_valid_password_reset_token(db, token=reset_data.token)
...
@router.patch("/reset-password-by-poweruser/", response_model=schemas.Message) # POST도 가능하지만 PATCH가 더 의미 명확
def reset_user_password_by_poweruser(
    reset_data: schemas.PasswordResetByPowerUserRequest, # 요청 본문: 대상 유저 이메일과 새 비밀번호
    db: Session = Depends(get_db), # 데이터베이스 세션
    current_power_user: models.User = Depends(security.get_current_active_superuser) # <-- 파워 유저 인증 및 권한 확인
):
    """
    ...
    target_user = crud.get_user_by_email(db, email=reset_data.target_user_email)
