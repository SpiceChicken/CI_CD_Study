...
def redirect_to_target(short_key: str, db: Session = Depends(get_db)):
    """
    단축 URL 조회 엔드포인트
    - 경로: GET /{short_key}
    - 매개변수: short_key (path)
    ...
    """
    url = db.query(URL)\
            .filter(URL.short_key == short_key, URL.is_active)\
            .first()
    if not url:
...
def deactivate_url(short_key: str, db: Session = Depends(get_db)):
    """
    URL 비활성화 엔드포인트
    - 경로: DELETE /{short_key}
    - 매개변수: short_key (path)
    ...
    """
    db_url = deactivate_url(db, short_key=short_key)
    if db_url is None:
        raise HTTPException(status_code=404, detail="URL not found")
...
def get_click_info(short_key: str, db: Session = Depends(get_db)):
    """
    단축 URL 클릭 정보 조회
...
def get_url_stats(short_key: str, db: Session = Depends(get_db)):
    ...
    - 반환: target_url, clicks, is_active
    """
    db_url = get_url_stats(db, short_key=short_key)
    if not db_url:
        raise HTTPException(status_code=404, detail="URL not found")
    return db_url
